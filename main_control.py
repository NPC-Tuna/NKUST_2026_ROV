...
# --- SCREEN 1080P LAYOUT CONFIGURATION ---
WIDTH, HEIGHT = 1920, 1080

# Sub-window layout specs (4-Quadrant Split)
HALF_W = WIDTH // 2       # 960
HALF_H = HEIGHT // 2      # 540
...
def main():
    ...
    # Launching in absolute 1920x1080 window
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption("NKUST ROV STATION - FIXED ASPECT RATIO HUD")
    ...
    while running:
        ...
        # --- KEYBOARD COMMAND PARSING & CAROUSEL DEBOUNCE ---
        ...
        # Use a true black background to blend the letterboxing
        screen.fill((0, 0, 0)) 

        # --- HUD Tactical Dividers ---
        # Draw tech-green dividing lines first, so frames draw over them if they overlap
        pygame.draw.line(screen, (0, 255, 0), (HALF_W, 0), (HALF_W, HEIGHT), 2)
        pygame.draw.line(screen, (0, 255, 0), (HALF_W, HALF_H), (WIDTH, HALF_H), 2)

        # --- DYNAMIC QUADRANT RENDERING WITH ASPECT RATIO CONSTRAINT ---
        
        # Helper function to get scaled frame to fit a target rectangle, keeping aspect ratio
        ...
        # 1. POSITION: Left Half (Controlled by cam_order[0])
        # Target area: (HALF_W, HEIGHT) = (960, 1080)
        target_key_0 = cam_order[0]
        surf_0, sw_0, sh_0 = get_scaled_frame(target_key_0, HALF_W, HEIGHT)
        if surf_0:
            # Center horizontally and vertically within the left half block
            ox = (HALF_W - sw_0) // 2
            oy = (HEIGHT - sh_0) // 2
            screen.blit(surf_0, (ox, oy))
        else:
            tag_err = font.render(f"WAITING FOR {target_key_0.upper()}...", True, (255, 60, 60))
            screen.blit(tag_err, (HALF_W // 2 - 120, HEIGHT // 2 - 14))

        # 2. POSITION: Top Right (Controlled by cam_order[1])
        # Target area: (HALF_W, HALF_H) = (960, 540)
        target_key_1 = cam_order[1]
        surf_1, sw_1, sh_1 = get_scaled_frame(target_key_1, HALF_W, HALF_H)
        if surf_1:
            # Center within the top-right block
            ox = HALF_W + (HALF_W - sw_1) // 2
            oy = (HALF_H - sh_1) // 2
            screen.blit(surf_1, (ox, oy))
        else:
            tag_err = font.render(f"WAITING FOR {target_key_1.upper()}...", True, (255, 60, 60))
            screen.blit(tag_err, (HALF_W + HALF_W // 2 - 140, HALF_H // 2 - 14))

        # 3. POSITION: Bottom Right (Controlled by cam_order[2])
        # Target area: (HALF_W, HALF_H) = (960, 540)
        target_key_2 = cam_order[2]
        surf_2, sw_2, sh_2 = get_scaled_frame(target_key_2, HALF_W, HALF_H)
        if surf_2:
            # Center within the bottom-right block
            ox = HALF_W + (HALF_W - sw_2) // 2
            oy = HALF_H + (HALF_H - sh_2) // 2
            screen.blit(surf_2, (ox, oy))
        else:
            tag_err = font.render(f"WAITING FOR {target_key_2.upper()}...", True, (255, 60, 60))
            screen.blit(tag_err, (HALF_W + HALF_W // 2 - 140, HALF_H + HALF_H // 2 - 14))

        # Draw source OSD tags on each viewport overlaying the screens
        ...
        screen.blit(tag_0, (10, 10))
        screen.blit(tag_1, (HALF_W + 10, 10))
        screen.blit(tag_2, (HALF_W + 10, HALF_H + 10))
