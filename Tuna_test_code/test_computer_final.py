...
def video_receiver_worker(port, dict_key, cam_name):
    ...
    try:
        ...
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 2097152) # enlarge receive buffer
    ...
    while running:
        ...
        if keys[pygame.K_e]:
            if not e_key_pressed:
                current_main_cam = cam_order[0]  # the camera currently in the large left panel

                # grab the raw frame quickly while holding the lock
                with frame_locks[current_main_cam]:
                    frame_to_save = shared_frames[current_main_cam]
                
                if frame_to_save is not None:
                    # build a timestamp (e.g. 20260607_153022)
                    timestamp = time.strftime("%Y%m%d_%H%M%S")
                    
                    # resolve this computer's Desktop path dynamically
                    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
                    filename = os.path.join(desktop_path, f"ROV_{current_main_cam.upper()}_{timestamp}.jpg")
                    
                    # save the clean raw OpenCV frame
